...
type RHFInputProps<T extends FieldValues = FieldValues> = Omit<ComponentProps<typeof Input>, 'label' | 'error' | 'required'> & {
  control: Control<T>
  name: Path<T>
...
export default function RHFInput<T extends FieldValues = FieldValues>({
  control,
  register,
  name,
  label,
